...
MIN_PF = -0.95    # Minimum PF limit (restricted to -0.9 at upwards)
    ...
    def control_power_factor(self, voltage, current, power_factor):
        ...
        try:
            if kw < KW_THRESHOLD:
                # Case 1: kW < 56000
                if voltage < 404:
                    threshold_current = (kw / voltage / sqrt3) + 18 + (voltage - 404) * 1.5
                elif voltage < 414:
                    threshold_current = (kw / voltage / sqrt3) + 18 + (voltage - 404) *0
                elif 414 <= voltage <=430:
                ...
                # Case 2: kW >= 56000
                if voltage < 404:
                    threshold_current = (kw / voltage / sqrt3) + (voltage - 413) * 2
                elif voltage < 413:
                    threshold_current = (kw / voltage / sqrt3) + (voltage-413)*1.2
                elif voltage < 426:
